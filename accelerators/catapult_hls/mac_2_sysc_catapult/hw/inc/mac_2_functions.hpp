...
#include "mac_2.hpp"
void mac_2_sysc_catapult::compute_req(uint32_t iter, uint32_t kalman_iters, uint32_t kalman_mat_dim, uint32_t constant_matrices_size, bool pingpong, bool out_pingpong)
{
...
void mac_2_sysc_catapult::compute(uint32_t iter, uint32_t kalman_iters, uint32_t kalman_mat_dim, 
                                    ...
                                    uint32_t Pp_base_address, uint32_t constant_matrices_size, bool pingpong, bool out_pingpong)
{
    ...
    FPDATA_WORD input_measurements_word[120];
    FPDATA input_measurements_fx[120];
    ...
    {
        if(pingpong)            
            input_measurements_word[k]=in_ping_rd.Pop().data[0];
        else
            input_measurements_word[k]=in_pong_rd.Pop().data[0];            
        int2fx(input_measurements_word[k],input_measurements_fx[k]);
            zk[k] = input_measurements_fx[k];
    ...
    if(iter > 0)
    {
        ...
        for (uint32_t k = 0; k < previous_states_elements; k++)
        {
            ...
            if(k < kalman_mat_dim)
            {
                X_cpp[k] = op_fx + 1; //REMOVE PLUS ONE
            }
    ...
    //     // Pp = Pp - K*H*Pp;
        ...
        #ifdef PRINT_STATEMENTS
        std::cout << "Writing Pp_cpp: "<< iter << "\n";
        print_matrix(Pp_cpp, kalman_mat_dim);
        #endif

        FPDATA send_output_array[30];

...
#define SLD_MAC_2 0x057
#define DEV_NAME "sld,mac_2_sysc_catapult"
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_MAC_2, DEV_NAME);
	if (ndev == 0) {
		printf("mac_2 not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
		iowrite32(dev, MAC_2_MAC_N_REG, mac_n);
		iowrite32(dev, MAC_2_MAC_VEC_REG, mac_vec);
		iowrite32(dev, MAC_2_MAC_LEN_REG, mac_len);
